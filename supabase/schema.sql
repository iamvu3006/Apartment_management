...
-- GIAI ĐOẠN DEMO (chưa có đăng nhập admin): cho phép mọi thao tác CRUD công khai.
-- ⚠️ Khi thêm trang đăng nhập admin, XOÁ policy "Public full access" bên dưới và
-- thay bằng các policy chỉ cho "authenticated" (đã viết sẵn, đang comment).
...
create policy "Public can upload room images (demo only)"
  on storage.objects for insert
  with check (bucket_id = 'room-images');

create policy "Public can delete room images (demo only)"
